...
export default function NavBarComponent() {
  ...
  return (
    <div
      className={`menu-container round-box pt-14 gap-9 items-center duration-150 ease-in-out ${
        isOpen ? "w-32" : "w-52"
      }`}
    >
      <div className="flex flex-row items-center justify-center gap-4 mb-7 min-w-full">
        <button onClick={openSideBar} className=" py-2">
          {isOpen ? (
            <ChevronLast className="min-w-6" />
          ) : (
            <ChevronFirst className="min-w-6" />
          )}
        </button>
        <h1
          className={`${!isOpen ? "block" : "hidden"} font-righteous text-xl`}
        >
          ROOMIE
        </h1>
      </div>
