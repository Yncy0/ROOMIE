...
const UserSchedulePage = () => {
  ...
  const transformData = (data, type) =>
    data.map((item) => {
      ...
      return {
        ...
        subject_code: isBooking ? item.subject_name || "Unknown Subject" : item.subject?.subject_code || "Unknown",
        ...
        course_name: isBooking ? item.course_name || "Unknown Course" : item.course?.course_name || "Unknown Course",

        course_year: isBooking ? item.course_year || "Unknown Year" : item.course?.course_year || "Unknown Year",
  ...
  const handleUserSelection = (user) => {
    ...
    const userSchedule = scheduleData.filter((event) => event.user_name === user)
  ...
  const handleFormSubmit = async () => {
    ...
    const formattedData = {
      ...
      time_out: formData.date ? `${formData.date}T${formData.time_out}:00` : null,
    }
    ...
    if (modalType === "booking") {
      ...
      try {
        if (selectedEvent) {
          ...
          delete formattedData.subject_id
          // Insert new booking
          const { data, error } = await supabase.from("booked_rooms").insert([formattedData])
      ...
      formattedData.time_in = dayjs(formData.time_in, "HH:mm").format("HH:mm:ss")
      formattedData.time_out = dayjs(formData.time_out, "HH:mm").format("HH:mm:ss")
      ...
      if (isToday) {
        const startDateTime = dayjs(`${todayDays} ${formattedData.time_in}`, "dddd HH:mm")
        const endDateTime = dayjs(`${todayDays} ${formattedData.time_out}`, "dddd HH:mm")
        ...
        formattedData.status = dayjs().isBefore(dayjs().day(formattedData.days)) ? "Incoming" : "Complete"
      }
    ...
    >
      ...
      <Box sx={{ padding: 3, color: "black", display: "flex" }}>
        ...
        <Box sx={{ flexGrow: 1, padding: "20px" }}>
          {(isBookingLoading || isScheduleLoading) ? (
            <CircularProgress />
          ) : selectedUser ? (
            <Box>
              <Typography
